...
5. Browse and select the `config/database/ims_db.sql` file provided in this repository.
...
2. Update your database and app settings.
   ```env
   DB_HOST=localhost
   DB_NAME=ims_db
   DB_USER=root
   ...
   APP_NAME=FEU Alabang Book Store Inventory Management System
   APP_URL=http://localhost/IM-SYSTEM
   APP_DEBUG=true
   ```
...
## Security Note
Do not commit real database credentials to your repository. Keep sensitive values in `.env` and use local-only values for development.
